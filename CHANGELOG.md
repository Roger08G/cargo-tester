...
- Se cierran los grupos de procesos también cuando termina correctamente su líder;
  terminación y drenaje tienen esperas acotadas para evitar bloqueos por tuberías
  heredadas. Los tests de salida masiva conservan su resultado con captura truncada.
- CI omite texto libre de panic y operandos, invalida reportes anteriores y sanea
  historial/estado antes de compilar. Las anotaciones no incluyen texto privado.
- Controles de terminal neutralizados y saneamiento de formatos npm.
- Lecturas de configuración, contexto fuente, historial y estado acotadas; contexto
  limitado a archivos Rust regulares del workspace. Reportes Unix con modo 0600.
- Bloqueo en el destino canónico con espera máxima de 10 segundos; los historiales
  locales corruptos no se sobrescriben y se rechazan escrituras mayores de 16 MiB.
- Argumentos persistidos revalidados, comparaciones de rendimiento separadas por
  target y timestamps malformados tratados sin panic.

### Optimización y cobertura

- Índice de líneas lineal y presupuesto global de indexación; se evitan copias
  innecesarias de salida y la colección de datos fuente está acotada.
- 61 pruebas pasan en Windows y 62 en Unix; cuatro casos ignorados por defecto
  son demostraciones intencionales y benchmarks manuales. Siete tests end-to-end.
- Benchmark documentado: en esta máquina, 24 tests mínimos son más lentos con
  aislamiento por proceso (910 ms frente a 54 ms); no se promete mayor velocidad.
- Dependencias bloqueadas actualizadas y auditoría RustSec sin avisos conocidos.
- La auditoría, medidas y límites están en docs/SECURITY_AUDIT_1.1.0.md.

### CI y distribución

- Tests, formato, Clippy, MSRV 1.85, empaquetado y uso real de cargo-tester
  verificados en CI antes de crear la rama y el tag.
- Se restaura la política de changelog exclusivo de ramas/tags de release.
- Checkout sin credenciales persistidas; limpieza explícita de reportes previos
  y pruebas sobre cada plataforma de distribución.
- Reintentos verifican SHA-256 y no sustituyen assets publicados diferentes.
- Release Production v1.1.0: instalador Windows, portables, fuentes y SHA256SUMS.
  Los ejecutables siguen sin firma Authenticode/notarización.
...
[1.1.0]: https://github.com/Roger08G/cargo-tester/compare/v1.0.0...v1.1.0
