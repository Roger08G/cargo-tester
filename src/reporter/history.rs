...
fn format_timestamp(unix_ms: u128) -> String {
    // humantime formats RFC3339 years up to 9999; persisted data is untrusted.
    if unix_ms > 253_402_300_799_999 {
        return "Invalid timestamp".to_owned();
    }
    let Some(timestamp) = UNIX_EPOCH.checked_add(Duration::from_millis(unix_ms as u64)) else {
        return "Invalid timestamp".to_owned();
    };
...
pub fn render_regressions(
    ...
    let lines = regressions
        ...
        .map(|regression| {
            format!(
                "{marker} {}: {} -> {} (+{:.1}%)",
                sanitize_text(&regression.full_name, config.privacy.redact),
...
mod tests {
    ...
    #[test]
    fn rejects_out_of_range_stored_timestamps() {
        assert_eq!(format_timestamp(u128::MAX), "Invalid timestamp");
